    ...
    <div className="bg-white rounded-lg shadow-md p-6 relative">
      ...
      {todos.length === 0 ? (
        ...
        <div className="space-y-4">
          {/* 내 할일 섹션 */}
          {pendingTodos.filter((todo) => todo.is_own_todo === true).length >
            0 && (
            <div>
              <h4 className="text-sm font-medium text-gray-700 mb-3 flex items-center">
                ...
                내 할일 (
                {
                  pendingTodos.filter((todo) => todo.is_own_todo === true)
                    .length
                }
                개)
              </h4>
              <div className="space-y-3">
                {pendingTodos
                  .filter((todo) => todo.is_own_todo === true)
                  .map((todo) => (
                    <div
                      key={todo.id}
                      className={`group border rounded-lg p-4 transition-shadow ${
                        todo.is_own_todo
                          ? "hover:shadow-md cursor-pointer"
                          : "bg-blue-50 border-blue-200 cursor-not-allowed"
                      }`}
                      style={{
                        borderColor: todo.is_own_todo
                          ? "var(--border-color)"
                          : "var(--blue-200)",
                      }}
                      onClick={(e) => {
                          ...
                          <div className="flex items-center space-x-2 mb-1">
                            <h4
                              className={`font-medium text-sm truncate ${
                                !todo.is_own_todo ? "text-blue-700" : ""
                              }`}
                              style={{
                                color: todo.is_own_todo
                                  ? "var(--text-primary)"
                                  : "var(--blue-700)",
                              }}
                            >
          ...
          {/* 친구 할일 섹션 */}
          {pendingTodos.filter((todo) => todo.is_own_todo === false).length >
            0 && (
            <div>
              <h4 className="text-sm font-medium text-blue-700 mb-3 flex items-center">
                ...
                친구의 할일 (
                {
                  pendingTodos.filter((todo) => todo.is_own_todo === false)
                    .length
                }
                개)
              </h4>
              <div className="space-y-3">
                {pendingTodos
                  .filter((todo) => todo.is_own_todo === false)
                  .map((todo) => (
                    <div
                      key={todo.id}
          ...
          {completedTodos.length > 0 && (
            <div className="pt-4">
              ...
              <div className="space-y-3">
                {/* 내 완료된 할일들 */}
                {completedTodos.filter((todo) => todo.is_own_todo === true)
                  .length > 0 && (
                  <div>
                ...
                {/* 친구 완료된 할일들 */}
                {completedTodos.filter((todo) => todo.is_own_todo === false)
                  .length > 0 && (
                  <div>
